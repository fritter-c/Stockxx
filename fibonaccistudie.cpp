...
void FibonacciStudie::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if ((m_dStartPrice != m_dEndPrice) or (m_qiStartQuote != m_qiEndQuote)){
        if ((m_tsVisual->XAtQuote(m_qiStartQuote) > 0) and (m_tsVisual->XAtQuote(m_qiEndQuote) > 0)){
            ...
            bool bCanFit {abs(m_tsVisual->XAtQuote(m_qiStartQuote) - m_tsVisual->XAtQuote(m_qiEndQuote)) > rAdvance};
            // 0% line
            ...
            if (m_dStartPrice < m_dEndPrice){
                ...
                if ((bLeftAlign) and (bCanFit)){
                    point10.rx() = point10.rx() - rAdvance;
                ...
                if ((bLeftAlign) and (bCanFit)){
                    point12.rx() = point12.rx() - rAdvance;
